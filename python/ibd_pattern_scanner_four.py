"""
ibd_pattern_scanner_four.py  - RESEARCH COPY of ibd_pattern_scanner.py
...
Two changes from production. Double Bottom stops competing for the label (see below), and the
vocabulary is made TOTAL, so a live base always carries one of the four names.

'Base' was the fallback when none of the four matched, and it fired on 59 live rows - 57 still
IN a base, and 58 of the 59 already carrying a layered reading that named them (Cup 44,
Consolidation 26, Cup+Handle 9, Flat Base 8). The scanner knew what they were and reported an
unusable name anyway. The hole is plain coverage: Cup takes 8-55% depth over 25-130 bars and
Consolidation 5-35%, so a base between those bands - or shorter than 25 bars, or deeper than
55% - matched nothing. Consolidation IS the residual in IBD's vocabulary, so making it total
is what the name already means.

IMPORTANT - the residual is applied to the NAMING CHAINS ONLY, via `nameConsol`, and NOT to
`isConsolidation`. Widening `isConsolidation` itself looks equivalent and is not: it feeds
`inBase`, which is read AFTER the breakout check has set `isBase = False`, so the flag is
stale by then and a base that broke out this bar still reports `inBase = True`. That leaks
into `prev_isBase` and `noNe` and suppresses the NEXT base's seed - measured 658 -> 649 seeds
and -4 exact events, with knock-on label and pivot changes on 12 events that have nothing to
do with the 'Base' fallback. The staleness is load-bearing, too: removing it (`inBase = isBase
or isHTF`) doubles seeding to 1259 and drops exact to 61. So leave `inBase` alone and name
around it.
...
OUTPUT_JSON_PATH = ROOT_DIR / "python" / "ibd_pattern_results_four.json"
...
def classify_candidate_base(highs, lows, top, low, count, end, lag=8):
    ...
    # `count > 200 or 5 <= dep <= 35` gate returned no name and no pivot for a candidate
    # base it had already located.
...
def scan_single_ticker(ticker: str, file_path: str, spy_close_series: pd.Series = None):
    ...
    try:
        ...
        for i in range(n):
            ...
            # Naming-only residual - see the module docstring for why this is NOT applied to
            # `isConsolidation` itself. Used by the two label chains and nothing else.
            ...
            # 'Base' is unreachable now: isDeepBase implies isBase and not isFlatBase,
            # which nameConsol already claims. Kept as a tripwire - if 'Base' shows up
            # in output again, the residual has been re-gated somewhere.
