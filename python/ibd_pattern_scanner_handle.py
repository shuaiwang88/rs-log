...
OUTPUT_JSON_PATH = ROOT_DIR / "python" / "ibd_pattern_results_handle.json"
...
# --- Cup+Handle gates ----------------------------------------------------------------
# Handle recall is the weak spot: 18 of 41 true Cup With Handle events are caught, and 14 of
# the misses are called plain Cup. Errors here are ASYMMETRIC and it is the recall side that
# is safe - a missed handle quotes the base top, which is too HIGH (the trade is merely late
# or skipped), while a false handle quotes the handle high, which is too LOW and reports a
# breakout that has not happened. So precision (currently 75%) is the constraint, not the
# target: any relaxation has to be read against it.
#
# `require_cup` and `tal_max` are the two structural additions; the rest are the port's
# existing constants pulled out so they can be swept.
HANDLE = {
    'bdep_lo': 20.0, 'bdep_hi': 50.0,   # base depth band the handle search runs inside
    'flat_guard_dep': 25.0,             # flat-base guard escapes above this depth
    'rdep_min': 12.0,                   # recent-window depth floor
    'in_top': 0.95,                     # handle low >= cupMid * this
    'hdep_lo': 5.0, 'hdep_hi': 30.0,    # handle depth band (Pine caps decline at 12%)
    'vol_mult': 1.15,                   # handle avg vol < ref * this (Pine wants 0.50)
    'slope_max': 0.60,                  # handle highs must not still be advancing
    'hd_ratio_max': 0.45,               # handle depth / cup depth
    'require_cup': False,               # Pine line 837, default ON there and never ported
    'use_tal': False,                   # time-at-low discriminator (see below)
    'tal_max': 0.30,
}
# Share of base bars whose LOW sits in the bottom quarter of the base's range. It is the
# strongest separator found between Cup With Handle and Cup Without Handle - AUC 0.324
# (0.676 inverted) at the event bar, 0.318 ten bars earlier - because a handle forms up near
# the top, so a base carrying one spends less of its life at the low.
#
# DEFAULTED OFF: it does not replicate. At tal <= 0.30 the 137-event benchmark reads 13
# claims and 13 true positives (precision 75% -> 100%, recall 44% -> 32%), which looks
# decisive until the same config is run over the 331 stocks on IBD's "Bases Forming" list,
# where precision goes 23% -> 20% and recall 26% -> 16%. With only 41 true handles in the
# benchmark and ~15 configs swept, 13/13 is inside what chance produces. The two sets also
# disagree at baseline (75% vs 23% precision), which says IBD's WEEKLY Cup With Handle is
# not the same object as this scanner's daily handle - so neither number should be trusted
# alone. Left switchable so the claim stays checkable.
...
def scan_single_ticker(ticker: str, file_path: str, spy_close_series: pd.Series = None):
    ...
    try:
        ...
        for i in range(n):
            ...
            # Handle-independent cup verdict, latched per base. Pine evaluates isCup BEFORE
            # the handle block and gates the handle on it (line 837); the port cannot reuse
            # its own `isCup` because that one is guarded by `not isCupH`, so asking for it
            # here would be circular. This recomputes the same depth/length bands without the
            # handle guard, and latches, matching Pine's `if detectCup or isCup[1]`.
            _cup_now = False
            if isBase and bTop and bLow and bDepPct is not None:
                if 25 <= bCount <= 130:
                    _cup_now = 8.0 <= bDepPct <= 55.0 and not isLikelyConsolidation
                elif 130 < bCount <= 250:
                    _cup_now = 15.0 <= bDepPct <= 45.0 and not isLikelyConsolidation
                elif bCount > 250:
                    _cup_now = (20.0 <= bDepPct <= 50.0
                                and not (bDepPct >= 30.0 and rDepPct < 25.0))
            if newBase:
                cup_latched_for_handle = False
            if _cup_now:
                cup_latched_for_handle = True

            # Time-at-low, the Cup+Handle vs Cup discriminator (see HANDLE above).
            handle_tal = None
            handle_tal_ok = True
            if isBase and bStart is not None and bTop and bLow and bTop > bLow:
                _seg = lows[bStart:i + 1]
                if len(_seg):
                    handle_tal = float(np.sum(_seg <= bLow + (bTop - bLow) * 0.25) / len(_seg))
                    if HANDLE['use_tal']:
                        handle_tal_ok = handle_tal <= HANDLE['tal_max']
            ...
            cupH_allowed = (not prevIsFlatBase or not isFlatBase) or (bDepPct is not None and bDepPct >= HANDLE['flat_guard_dep'])
            # Every ENTRY gate recorded separately, so a missed handle can be attributed to
            # the one test that actually blocked it instead of guessed at. The entry gates
            # reject the base before a handle window is even measured, which makes them
            # invisible to any diagnostic that only looks at the handle itself.
            hg_entry = {
                'is_base': bool(isBase and bTop and bLow and cupMid),
                'bcount20': bool(bCount >= 20),
                'flat_guard': bool(cupH_allowed),
                'bdep_20_50': bool(bDepPct is not None and HANDLE['bdep_lo'] <= bDepPct <= HANDLE['bdep_hi']),
                'rdep_gt12': bool(rDepPct > HANDLE['rdep_min']),
            }
            handle_gates = dict(hg_entry)
            if isBase and bTop and bLow and cupMid and bCount >= 20 and cupH_allowed and (bDepPct is not None and HANDLE['bdep_lo'] <= bDepPct <= HANDLE['bdep_hi']) and rDepPct > HANDLE['rdep_min']:
                ...
                inTop = (L12 >= cupMid * HANDLE['in_top'])
                max_hDep = 20.0 if bCount > 250 else HANDLE['hdep_hi']
                depOk_h = (HANDLE['hdep_lo'] <= hDep <= max_hDep)
                ...
                volOk_h = (ref_vol is None or ref_vol <= 0) or (handle_avg_vol < ref_vol * HANDLE['vol_mult'])
                ...
                if len(_hw) >= 4 and H12 > 0:
                    ...
                    slopeOk_h = (_hsl <= HANDLE['slope_max'])
                hdRatio = hDep / bDepPct if bDepPct and bDepPct > 0 else 1.0
                # Pine line 837: `requireCupOK = not i_requireCupForHandle or isCup or
                # isDoubleBottom`, default ON. The port never carried this across - its
                # isCupH is computed BEFORE isCup and independent of it. Off by default here
                # because turning it on can only remove handles; measured either way below.
                cupReq_h = (not HANDLE['require_cup']) or bool(cup_latched_for_handle)
                handle_gates.update({
                    'in_top': bool(inTop), 'hdep': bool(depOk_h), 'vol': bool(volOk_h),
                    'slope': bool(slopeOk_h), 'below_top': bool(H12 < bTop * 1.02),
                    'hd_ratio': bool(hdRatio <= HANDLE['hd_ratio_max']),
                    'require_cup': bool(cupReq_h),
                    'time_at_low': bool(handle_tal_ok),
                    '_hDep': float(round(hDep, 2)), '_hdRatio': float(round(hdRatio, 3)),
                    '_slope': float(round(_hsl, 3)) if _hsl is not None else None,
                    '_volRatio': float(round(handle_avg_vol / ref_vol, 3)) if (ref_vol and ref_vol > 0) else None,
                    '_tal': handle_tal,
                })
                if (inTop and depOk_h and volOk_h and slopeOk_h and H12 < bTop * 1.02
                        and cupReq_h and handle_tal_ok
                        and hdRatio <= HANDLE['hd_ratio_max']):
                        isCupH = True
                        cupHandlePivot = H12
