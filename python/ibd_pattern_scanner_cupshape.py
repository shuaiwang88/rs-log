"""
ibd_pattern_scanner_dbortho.py  - RESEARCH COPY of ibd_pattern_scanner.py

Experiment: make Double Bottom an ORTHOGONAL attribute instead of a competing label, so a
base can be a Cup+Handle / Cup / Flat Base / Consolidation AND carry a double bottom.

What changed (detection itself is untouched - the W test is bit-for-bit the production one):
  * 'Dbl Bottom' is out of both label priority chains, so it never wins `pattern_name`.
  * It no longer sets a buy point - neither `pivRef` nor the breakout `active_pivot`.
  * In the layered readings it prices off the BASE TOP and is demoted in PATTERN_RANK, so it
    folds into the host pattern's `also_reads_as` rather than owning the entry.
  * `is_double_bottom` / `db_middle_peak` are emitted as attributes alongside the real label.

Why the middle peak stopped being the buy point - IBD doctrine says a double bottom is bought
at the middle peak of the W, so this is a deliberate departure, made because the detector has
not earned it. Measured 2026-08-04:
  * Of 331 stocks on IBD's own "Bases Forming" list, the scanner led with Dbl Bottom on 33.
    IBD calls NONE of them a double bottom (17 Consolidation, 11 Flat Base, 5 Cup).
  * On those 33 the middle-peak pivot ran a median 8.4% BELOW IBD's buy point, 28 of 33 low
    by more than 3%, worst -26.0% (COF). The next reading in the list - the base top - had a
    median error of 0.0% and landed within 3% on 30 of 32.
  * On the 140-event Breakaway Gap benchmark the DB reading scores 1 true positive against 22
    false, and `pattern_name` never once settles on 'Dbl Bottom'.
Quoting a buy point too LOW is the expensive error (it reports a breakout that has not
happened and buys into overhead supply), so an unreliable sub-structure must not lower the
price. See the note above `altPat`.
...
Scans all ticker parquet data in `ticker_cache/` to detect MarketSmith / IBD patterns:
  ...
  3. Cup
  4. Cup + Handle
  5. High Tight Flag (HTF)
  6. 6-Wk Flat Base
  (+ Double Bottom and VCP as sub-pattern attributes that qualify any of the above)
...
OUTPUT_JSON_PATH = ROOT_DIR / "python" / "ibd_pattern_results_cupshape.json"
...
# --- Cup shape gates -----------------------------------------------------------------
# Switches so each gate can be measured on its own and as a set. pine/drw_pattern.pine is
# the authority for the first four; the last two are additions it does not have.
# Change these as a SET, not one at a time - measuring a single gate with the others wide
# open has previously overstated its cost badly (see the HTF note further down).
CUP_SHAPE = {
    'pine_third':     False,    # condThird     - middle third at/below the cup midpoint
    'pine_third_two': False,    # condThirdTwo  - first quarter at/above the midpoint
    'pine_abs_pos':   False,    # condAbsolutePos - price recovered to the midpoint
    'pine_min_len':   False,    # condMinLength - >= 30 bars (port used 25)
    'persist':        False,    # Pine's `cond and cond[1]` two-bar persistence
    'latch':          False,    # Pine line 777: once a cup, a cup for the rest of the base
    'v_reject':       False,   # NOT in Pine: reject V bottoms by time spent at the low
    'symmetry':       False,   # NOT in Pine: two-sided low-position balance (measured: useless)
    'low_pos':        True,   # NOT in Pine: one-sided - the low must not sit in the first quarter
}
# Share of base bars whose LOW sits in the bottom quarter of the base range. A U spends many
# bars down there; a V touches once and leaves.
CUP_V_MIN_TIME_AT_LOW = 0.10
# Shorter side of the cup over the longer side, measured in bars either side of the low.
# Measured AUC 0.604 at T-0 but 0.502 at T-10 - i.e. nothing that survives to the moment the
# call is actually made. Kept switchable so the claim stays checkable, defaulted off.
CUP_MIN_SYMMETRY = 0.30
# Where the low sits along the base, 0.0 = at the start, 1.0 = at the end. This is the same
# idea as symmetry but WITHOUT folding the two sides together, and folding is what destroyed
# it: a Consolidation drops early and drifts (median 0.24), a Cup bottoms mid-base (median
# 0.46-0.55). AUC 0.659 at T-0 and 0.680 at T-10 - the only feature tested here that both
# separates and holds up 10 bars before the breakout.
CUP_MIN_LOW_POS = 0.30
...
def scan_single_ticker(ticker: str, file_path: str, spy_close_series: pd.Series = None):
    ...
    try:
        ...
        # Previous bar's cup-shape verdict, for Pine's `condThird and condThird[1]`
        # persistence. Seeded True so the first qualifying bar of a base is not penalised for
        # having no history - Pine's `[1]` on a var that has never been set reads its initial
        # value, which for these is the `false` declared at the top of the block; seeding True
        # instead only affects the single first bar and avoids a one-bar lag on every base.
        prev_shape_now = True
        cup_shape_latched = False
        ...
        for i in range(n):
            ...
            # --- CUP SHAPE, ported from pine/drw_pattern.pine lines 735-774 --------------
            # The port's `isCup` tests DEPTH and LENGTH and nothing else - its only shape gate
            # (`has_u_shape`) is dead, disabled behind `bCount > 999`. So any base 8-55% deep
            # over 25-130 bars is a Cup, which is why Cup over-claims (73 detections against
            # 50 true events) and swallows 18 of the 26 true Consolidations. The Pine it is a
            # port of has three real geometry gates that were never carried across.
            #
            # Pine indexes `close[k]` k bars BACK, and `baseCount := bar_index - startBaseBar`,
            # so k in [tier, baseCount - tier] is the base's MIDDLE THIRD and k in
            # [3*fourth, baseCount] is its OLDEST QUARTER. Pine's `/` on two ints truncates,
            # so `//` here is the faithful reading (see PINE_QUIRKS in tv_pattern_scanner.py
            # for the same ambiguity).
            cond_third = cond_third_two = cond_abs_pos = cond_min_len = True
            cup_v_ok = cup_sym_ok = True
            cup_shape_dbg = None
            if isBase and bTop and bLow and cupMid and bStart is not None and i > bStart:
                _bc = i - bStart                       # Pine's baseCount
                _tier, _fourth = _bc // 3, _bc // 4
                if _tier > 0:
                    # condThird: >=90% of the middle third's closes sit at or BELOW the cup
                    # midpoint. This is the rounded-bottom test - it says the base's low is
                    # centred in TIME, not tacked onto one end.
                    _mid = closes[i - (_bc - _tier): i - _tier + 1]
                    cond_third = (np.sum(_mid <= cupMid) / _tier) >= 0.90
                if _fourth > 0:
                    # condThirdTwo: >=30% of the OLDEST quarter's closes sit at or ABOVE the
                    # midpoint - the cup must start high and have a real left side, rather
                    # than beginning already down in the hole.
                    _first = closes[i - _bc: i - 3 * _fourth + 1]
                    cond_third_two = (np.sum(_first >= cupMid) / _fourth) >= 0.30
                # condAbsolutePos: price has climbed back to at least the cup midpoint.
                cond_abs_pos = highs[i] >= cupMid
                # condMinLength: Pine wants 30 bars, the port accepted 25.
                cond_min_len = _bc >= 30

                # --- beyond Pine: V-shape and symmetry ------------------------------------
                # Pine's condThird does NOT reject a V. Work it through: on a symmetric V the
                # middle third still sits below the midpoint the whole way, so a V passes the
                # rounded-bottom test cleanly. What separates a U from a V is TIME SPENT at
                # the low, so measure that directly - the share of base bars whose low is in
                # the bottom quarter of the base's range.
                _lo_seg = lows[bStart:i + 1]
                _rng = bTop - bLow
                if _rng > 0 and len(_lo_seg):
                    _q = bLow + _rng * 0.25
                    cup_v_ok = (np.sum(_lo_seg <= _q) / len(_lo_seg)) >= CUP_V_MIN_TIME_AT_LOW
                # Symmetry: where the low sits along the base. IBD cups are roughly balanced;
                # a base whose low is jammed against either end is a descent or an ascent, not
                # a cup. Reported as the shorter side over the longer.
                _lb = int(np.argmin(_lo_seg)) if len(_lo_seg) else 0
                _left, _right = _lb, (len(_lo_seg) - 1 - _lb)
                if max(_left, _right) > 0:
                    cup_sym_ok = (min(_left, _right) / max(_left, _right)) >= CUP_MIN_SYMMETRY
                # Raw RATIOS, not just the pass/fail bits, so the features can be checked for
                # separability before any threshold is argued over.
                cup_shape_dbg = {
                    'third': float(np.sum(_mid <= cupMid) / _tier) if _tier > 0 else None,
                    'third_two': float(np.sum(_first >= cupMid) / _fourth) if _fourth > 0 else None,
                    'abs_pos': bool(cond_abs_pos),
                    'bars': int(_bc),
                    'time_at_low': float(np.sum(_lo_seg <= _q) / len(_lo_seg)) if (_rng > 0 and len(_lo_seg)) else None,
                    'sym': float(min(_left, _right) / max(_left, _right)) if max(_left, _right) else None,
                    'low_pos': float(_lb / (len(_lo_seg) - 1)) if len(_lo_seg) > 1 else None,
                    'depth': float(bDepPct) if bDepPct is not None else None,
                }

            # Pine requires the shape to hold on THIS bar and the previous one
            # (`condThird and condThird[1] and condThirdTwo and condThirdTwo[1]`), which stops
            # a single bar's close flickering the cup on and off.
            cup_lowpos_ok = True
            if CUP_SHAPE['low_pos'] and cup_shape_dbg and cup_shape_dbg.get('low_pos') is not None:
                cup_lowpos_ok = cup_shape_dbg['low_pos'] >= CUP_MIN_LOW_POS

            _shape_now = ((cond_third or not CUP_SHAPE['pine_third'])
                          and (cond_third_two or not CUP_SHAPE['pine_third_two'])
                          and (cond_abs_pos or not CUP_SHAPE['pine_abs_pos'])
                          and (cond_min_len or not CUP_SHAPE['pine_min_len'])
                          and (cup_v_ok or not CUP_SHAPE['v_reject'])
                          and (cup_sym_ok or not CUP_SHAPE['symmetry'])
                          and cup_lowpos_ok)
            _shape_bar = (_shape_now and prev_shape_now) if CUP_SHAPE['persist'] else _shape_now
            prev_shape_now = _shape_now

            # LATCH, and this is the whole ballgame. Pine line 777 is
            # `if detectCup or isCup[1] : isCup := true` - once the geometry qualifies ANYWHERE
            # in the base, the cup stays a cup for the rest of it. Read per-bar instead, these
            # same gates cost 19 exact events (76 -> 57) and drop Cup recall from 76% to 24%,
            # because condThird measures a window that keeps sliding right as the base grows:
            # the qualifying-bar count stops rising while the denominator does not, so a
            # FINISHED cup scores worse than a half-built one. That decay is a known property
            # of this Pine (it is why the TV port names from a latch too) and it is why the
            # test must be evaluated as "did this base ever look like a cup", not "does it
            # look like one right now".
            if newBase:
                cup_shape_latched = False
            if _shape_bar:
                cup_shape_latched = True
            cup_shape_ok = cup_shape_latched if CUP_SHAPE['latch'] else _shape_bar
            ...
            if isBase and bTop and bLow and not isCupH:
                # The dead `has_u_shape` gate is replaced by the Pine's own geometry (see the
                # CUP SHAPE block above). The old gate measured the second half of `bCount`,
                # which is inflated relative to the real base length, so the window it looked
                # at was not the cup's right side - that is why it rejected more true Cups
                # than false ones and got switched off. The Pine tests the middle third and
                # the first quarter of the base instead, which do not depend on that.
                cup_ok = cup_shape_ok
                if cup_ok:
                    if (25 <= bCount <= 130):
            ...
            # both - which is the whole point of the change. `isDB` is now the independent
            # verdict and feeds only `inBase` (where it is redundant, since the W test already
            # requires isBase) and the reported attribute.
            ...
            isCup_ind = False
            # The independent reading carries the same shape gate as `isCup`, so the layered
            # readings cannot advertise a Cup the primary chain has just rejected as one.
            if isBase and bTop and bLow and cup_shape_ok:
                if (25 <= bCount <= 130):
            ...
            isConsolidation = isBase and (
                ...
                # being a consolidation. Without this a base that is only a Double Bottom would
                # fall through the (now DB-less) label chain to a bare 'Base'.
            ...
            state = {
                ...
                'altPat': tuple(
                    (nm, pv) for nm, ok, pv in (
                        ...
                        # PATTERN_RANK below, this makes the reading fold into whichever
                        # base-top pattern owns the entry, so it surfaces as
                        # `also_reads_as: ['Dbl Bottom']` beside Flat Base / Cup /
                        # Consolidation instead of displacing them at a lower buy point.
                        # Measured: over the 140 benchmark events the leading reading's buy
                        # point goes from 33 quoted low by >3% to 23, and within-3% from 88 to
                        # 95, with primary exact/broad and the headline pivot all unchanged.
        ...
        if (latest['pOn'] and (latest['pCode'] > 0)) or patterns:
            ...
            result = {
                ...
                # `db_middle_peak` is IBD's textbook buy point for a W and is kept for
                # drawing and for anyone who wants it, but it is deliberately NOT the quoted
                # pivot - see the module docstring for the measurement behind that.
