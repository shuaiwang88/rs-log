...
def scan_single_ticker(ticker: str, file_path: str, spy_close_series: pd.Series = None):
    ...
    try:
        ...
        for i in range(n):
            ...
            # U-shape check (needed by Cup vs Consolidation disambiguation)
            has_u_shape = False
            if isBase and bTop and bLow and bCount >= 30:
                mid_bar = i - bCount // 2
                if mid_bar >= 0:
                    second_half_high = np.max(highs[mid_bar:i+1])
                    has_u_shape = second_half_high >= bTop * 0.90

            # DETECTION ORDER: Cup+H first (most specific), then Cup, then Consolidation
            isCupH = False
            cupHandlePivot = None
            ...
            if isBase and bTop and bLow and cupMid and bCount >= 20 and (not prevIsFlatBase or not isFlatBase) and (bDepPct is not None and 20.0 <= bDepPct <= 50.0) and rDepPct > 12:
                # Handle length: 5-25 days depending on base length
                ...
                # IBD Rule: Handle must be in upper portion of cup
                handle_upper_check = (L12 >= cupMid * 0.60)
                
                if inTop and depOk_h and H12 < bTop * 1.02 and handle_upper_check:
            ...
            isCup = False
            if isBase and bTop and bLow and not isCupH and has_u_shape:
                if (25 <= bCount <= 130):
            ...
            # 7. Consolidation (after Cup/Cup+Handle detection)
            # IBD: TOO DEEP to be a flat base (>15%), NOT a clear cup or saucer
            isConsolidation = isBase and (
                (bCount > 250 and not isCup and not isCupH) or 
                (bDepPct is not None and bDepPct > 15.0 and bDepPct <= 50.0 
                 and not isCupH and not isFlatBase and not isDB
                 and not has_u_shape)
            )
            ...
            # 2. Ascending Base Detection (Disabled - rare pattern with only 5 ground truth events)
            isAscendingBase = False
            
            # 3. Double Bottom Detection (W-shape symmetry)
            # IBD Rule: Second leg MUST undercut first leg (critical requirement)
            isDB = False
            dbMiddlePivot = None
            dbMaxBars = 85
                ...
                for hp_i in range(min(5, len(aHP_list) - 1)):
                    for hp_j in range(hp_i + 1, min(len(aHP_list), hp_i + 5)):
                        ...
                        if l1_candidates and l2_candidates:
                            ...
                            # IBD Rule: Second leg MUST undercut first leg
