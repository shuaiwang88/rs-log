...
OUTPUT_JSON_PATH = ROOT_DIR / "python" / "ibd_pattern_results_life.json"
...
# --- Base lifecycle -------------------------------------------------------------------
# The state machine loses bases outright, and that shows up everywhere downstream: 4 of the
# 20 missed handles have no base alive at all, the cup-shape work stranded 55 rows into
# 'Base'/'None', and 5 benchmark events carry no primary pattern. A base that dies quotes
# nothing, or leaves a stale level behind.
#
# `bh_window` is the one the DVA post-mortem points at: a successor base's seed high has to
# still be among the N most recent pivot highs when `noNe` finally clears, and with N=3 it
# ages out first. `kill_runaway` is a PORT ADDITION - neither drw_pattern_scanner.pine (its
# source, line ~275) nor drw_pattern.pine (line 1148) kills a base for running above its top;
# both use depth and length only.
LIFE = {
    'bh_window': 3,        # pivot must be among the N most recent pivot highs
    'kill_runaway': True,  # close > bTop * runaway_mult kills the base (not in either Pine)
    'runaway_mult': 1.40,
    'reseed_same_bar': False,  # let a new base seed on the bar the old one dies
}
# MEASURED 2026-08-04, all defaulted to the production values because none of it replicates:
#
#   bh_window 3 -> 4/5/6      EXACTLY zero change - 667 base seeds either way, byte-identical.
#                             `bH` compares the candidate pivot's PRICE against the N most
#                             recent pivot-high prices, and the candidate at i-pivLag IS the
#                             newest of them, so only aHP_list[0] can ever match. The DVA
#                             post-mortem blaming "aged out of the three most recent pivot
#                             highs" cannot be the mechanism; widening the window is inert.
#   reseed_same_bar           +1 exact, +1 pivot-level, low-by->3% 16 -> 14 on the 137-event
#                             benchmark - every column improving at once, which is unusual -
#                             but -1 exact on the 331 IBD "Bases Forming" names, and it moves
#                             only 29 labels and 18 pivots in 2325 rows. Not significant.
#                             Note it needs `newBase_nn` (newBase WITHOUT noNe): `noNe` asks
#                             whether a base was live pivLag bars ago, which is always true on
#                             a kill bar, so newBase and "the old base just died" are mutually
#                             exclusive by construction and no reordering can bridge them.
#   kill_runaway OFF          -1 exact and -1 level, but low-by->10% 7 -> 5 and median error
#                             0.06% -> 0.02%. A wash, judged by [[pivot-over-label]]'s
#                             asymmetry. runaway_mult 1.60 is clearly worse (low>3% 16 -> 20).
    ...
    try:
        ...
        for i in range(n):
            ...
            if len(aHP_list) >= 3 and i >= pivLag:
                ...
                recent_hp_prices = [p for b, p in aHP_list[:LIFE['bh_window']]]
                ...
                nb_gates = {'bH': bool(bH), 'bPH': bool(bPH), 'lUp': bool(lUp),
                            'dep': bool(dep), 'noAb': bool(noAb), 'noNe': bool(noNe)}
                # Same-bar reseed needs this variant. `noNe` asks whether a base was live
                # pivLag bars ago, which is ALWAYS true on the bar a base dies - so `newBase`
                # and "the old base just died" are mutually exclusive by construction, and no
                # reordering of the seeding block can bridge them. Dropping noNe is sound
                # only in the post-kill retry, where the old base is known to be dead.
                newBase_nn = bH and bPH and lUp and dep and noAb

            prev_isBase = history_state[-1]['inBase'] if history_state else False
            
            # `reseed_same_bar` lets a qualifying pivot start a base on the same bar the old
            # one is killed. Without it the successor has to wait for `noNe` (no base pivLag
            # bars ago) to clear, and by then its seed high has usually aged out of the
            # `bh_window` most recent pivot highs - the documented DVA failure, where the
            # successor passed five of six gates and lost only on `noNe`.
            ...
            if isBase and bTop is not None:
                if lows[i] < bTop * (1.0 - bdF):
                    kill_reason = 'depth'
                elif bCount > bLenB:
                    kill_reason = 'length'
                elif LIFE['kill_runaway'] and closes[i] > bTop * LIFE['runaway_mult']:
                    kill_reason = 'runaway'
                if kill_reason is not None:
                    isBase = False
                    # Same-bar reseed. The seeding block above runs BEFORE this kill, so a
                    # qualifying pivot on the bar the old base dies is discarded by
                    # `not prev_isBase` and has to wait for `noNe` to clear - the documented
                    # DVA failure. Retrying here is the only point where the old base is
                    # actually known to be dead. `noNe` still has to have been satisfied, so
                    # this does not admit a base nested inside a live one.
                    if LIFE['reseed_same_bar'] and newBase_nn:
                        piv_idx = i - pivLag
                        bTop = highs[piv_idx]
                        bTopBar = piv_idx
                        bLow = L25
